...
const log = (lvl, data) => {
  ...
  const time = levels[lvl].time
    ? levels[lvl].time(getTime())
    : getTime()
  const level = levels[lvl].level.decor
    ? levels[lvl].level.decor(levels[lvl].level.title)
    : levels[lvl].level.title
  const name = levels[lvl].name
    ? levels[lvl].name(`[ ${filename} ]`)
    : `[ ${filename} ]`

  const msg = levels[lvl].msg
    ? levels[lvl].msg(colorAddress(message))
    : message
